...
const CompliancePlan = () => {
  ...
  const toggleStep = async (step: Step) => {
    ...
    if (allDone && plan) {
      ...
      toast({ title: "Plan completed!", description: "All steps have been marked as done." });
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setSteps((prev) =>
                              prev.map((s) =>
                                s.id === step.id ? { ...s, assigned_to: e.target.value } : s
                              )
                            )
                          }
                          onBlur={(e) => updateStepField(step.id, "assigned_to", e.target.value || null)}
                          className="w-full px-2 py-1.5 rounded bg-secondary/50 border border-border/50 text-foreground text-xs outline-none focus:border-primary/50"
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            setSteps((prev) =>
                              prev.map((s) =>
                                s.id === step.id ? { ...s, due_date: val } : s
                              )
                            );
                            updateStepField(step.id, "due_date", val);
                          }}
                          className="w-full px-2 py-1.5 rounded bg-secondary/50 border border-border/50 text-foreground text-xs outline-none focus:border-primary/50"
                        />
                      </div>
                    ...
                    <div className="mt-3">
                      ...
                      <textarea
                        ...
                        onChange={(e) =>
                          setSteps((prev) =>
                            prev.map((s) =>
                              s.id === step.id ? { ...s, memo: e.target.value } : s
                            )
                          )
                        }
                        onBlur={(e) => updateStepField(step.id, "memo", e.target.value || null)}
                        rows={2}
                        className="w-full px-2 py-1.5 rounded bg-secondary/50 border border-border/50 text-foreground text-xs outline-none focus:border-primary/50 resize-none"
                        placeholder="Add notes, findings, or decisions..."
                      />
                    </div>
